...
function cambiadepartamento() {
    form.idprovincia='';
    router.get('Enfermera', {dep: form.iddepartamento}, {preserveState: true, preserveScroll: true, only: ['prov']});  
}
function modalo(){
    updatemodal.value=false;
    form.clearErrors();
    form.reset();
    $('#registroenfermera').modal('show'); 
}
function modalupdate(enfermera){
    updatemodal.value=true;
    form.clearErrors();
    form.reset();
    form.id=enfermera.id;
    form.iddepartamento=enfermera.iddepartamento;
    cambiadepartamento();
    form.idrol=3;
    form.idprovincia=enfermera.idprovincia;
    form.name=enfermera.name
    form.genero=enfermera.genero;
    form.ci=enfermera.ci;
    form.fechanacimiento=enfermera.fechanacimiento;
    form.matricula=enfermera.matricula;
    form.telefono=enfermera.telefono;
    form.telfamiliar=enfermera.telfamiliar;
    form.dir=enfermera.dir;
    form.email=enfermera.email; 
    form.foto=enfermera.foto;
    $('#registroenfermera').modal('show'); 
}
const closeModal=()=>{ 
    updatemodal.value=false;
    form.reset();
    form.clearErrors(); 
    $('#registroenfermera').modal('hide');  
};
const guardarDatos=()=>{
    form.post(route('RegEnfermera'),{onSuccess:()=>{ok('Creado correctamente')}  });
};
    ...
    alerta.fire({
        ...
        (result)=>{
            if(result.isConfirmed){ 
                form.id=id;
                form.post(route('EliminarEnfermera'),{onSuccess:()=>{ok('Eliminado correctamente')}  });
            } 
        }
    );
    }; 
...
watch(searchField, debounce(() => { 
    router.get('/Enfermera', {search: searchField.value}, {preserveState: true, preserveScroll: true, only: ['lista']})
}, 300));
</script>
...
<template>
                            ...
                            <div class="card-body">
                                        ...
                                        <div class="col">
                                            <div class="doctor-table-blk">
                                                <div class="col-12 m-3">
                                                      <h3>Listado general de enfermeras</h3>
                                                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table border-0 custom-table comman-table datatable mb-0">
                                        ...
                                        <tbody> 
                                            <tr v-for="enfermera in lista.data" :key="enfermera.id"> 

                                                <td class="profile-image"><img width="28"
                                                                                height="28" :src="enfermera.foto?enfermera.foto:'assets/img/user.jpg'"
                                                                                class="rounded-circle m-r-5" alt="">{{ enfermera.name}}</td> 
                                                <td class="align-middle" style="text-align: center;">{{ enfermera.ci }}</td>
                                                <td class="align-middle" style="text-align: center;">{{ enfermera.fechanacimiento }}</td>
                                                <td class="align-middle" style="text-align: center;">{{ enfermera.matricula }}</td>
                                                <td class="align-middle" style="text-align: right;">{{ enfermera.email }}</td>
                                                <td class="align-middle" style="text-align: right;">{{ enfermera.telefono }}</td> 

                                                <td class="text-end">
                                                                        <button @click="modalupdate(enfermera)"
                                                                            class="btn btn-sm btn-white text-success me-2"><i
                                                                                class="far fa-edit me-1"></i> Editar</button>

                                                                        <button @click="EliminarDatos(enfermera.id)"
    ...
    <!-- -------------------modals---------------------- -->
    <div class="modal custom-modal modal-bg  fade invoices-preview" id="registroenfermera" role="dialog">
